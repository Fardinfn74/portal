...
export function LoginScreen({ onComplete }: LoginScreenProps) {
  ...
  useEffect(() => {
    ...
    timers.push(window.setTimeout(() => setStatus('authenticating session'), 2300));
    timers.push(window.setTimeout(() => {
      setStatus('login accepted');
      sounds.playLogin();
    }, 2720));
    timers.push(window.setTimeout(onComplete, 3180));
  ...
  return (
    <section className="relative flex h-full w-full items-center justify-center overflow-hidden bg-black">
      ...
      <motion.div
        ...
        className="relative w-[min(92vw,420px)] rounded-[8px] border border-white/20 bg-black/90 p-6 shadow-[0_0_30px_rgba(255,255,255,0.15)] backdrop-blur-xl"
      >
        <div className="mb-8 flex flex-col items-center">
          <div className="relative mb-6 h-32 w-32 overflow-hidden rounded-full border-2 border-white/20 bg-white/5 shadow-[0_0_30px_rgba(255,255,255,0.1)]">
            <img 
              src="/profile.png" 
              alt="Profile" 
